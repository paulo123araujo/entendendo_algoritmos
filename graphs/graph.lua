...
function graph.create(V)
	local g = {}
	setmetatable(g, graph)

	g.vertexList = require("graphs.list").create()
	g.adjList = {}
	for v = 0, V - 1 do
		g.vertexList:add(v)
		g.adjList[v] = require("graphs.list").create()
	end
...
function graph:addVertexIfNotExists(v)
	if self.vertexList:contains(v) then
		...
		self.adjList[v] = require("graphs.list").create()
		return true
	end
end
...
function graph:edges()
	local list = require("graphs.list").create()
